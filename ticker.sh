#!/bin/bash
# ============================================================================
# GIGACPO Ticker Intelligence Bootstrapper (V30.6.10)
# Automatically creates venv, syncs dependencies, and handles Playwright
# Ported from news.sh for Cross-Channel Parity
# ============================================================================

set -e

echo "============================================================"
echo " GIGACPO TICKER INTELLIGENCE - LINUX STARTUP"
echo "============================================================"
...
SOE_LOG="/home/jonesy/ticker_intel.log"
"$VENV_PYTHON" -u engine/ticker_dashboard.py "$@" 2>&1 | tee -a "$SOE_LOG"

dispatch_ts=$(date "+%a %b %d %I:%M:%S %p %Z %Y")
echo "$dispatch_ts: Ticker Dashboard Refreshed" | tee -a "$SOE_LOG"
