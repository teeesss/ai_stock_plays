"""
GIGACPO Pipeline Orchestrator [V28]
...
class PipelineOrchestrator:
    ...
    def process(self):
        ...
        print(
            f"--- GIGACPO Pipeline: {self.terminal_type.upper()} ({len(target_tickers)} tickers) ---"
        )
        ...
        for symbol in target_tickers:
            ...
            if not r_info.get("Company") or r_info.get("Company") == symbol:
                long_name = entry.get("financials", {}).get("price", {}).get(
                    "longName"
                ) or entry.get("financials", {}).get("summaryProfile", {}).get("longName")
            ...
            eps26 = next(
                (
                    t.get("earningsEstimate", {}).get("avg", {}).get("raw")
                    for t in trend
                    if t.get("period") in ["0y", "y"]
                ),
                None,
            )
            eps27 = next(
            ...
            price_val = fin.get("price", {}).get("regularMarketPrice", {}).get(
                "raw"
            ) or summary.get("regularMarketPrice", {}).get("raw", 0)

            # Dynamic Upside from OpenBB or Human Research
            raw_upside = obb.get("analyst_implied_upside_pct")
            upside_val = (
                (raw_upside / 100) if raw_upside is not None else 0.25
            )  # Default 25% if unknown
            ...
            if not growth_str and symbol in research_raw:
                growth_str = (
                    research_raw[symbol].get("human_research", {}).get("Rev Growth Est", "")
                )
            ...
            if growth_str:
                try:
                    rev_num = (
                        float(str(growth_str).replace("%", "").strip())
                        if "%" in str(growth_str)
                        else float(growth_str)
                    )
                except:
                    rev_num = 0
            ...
            stats_map[symbol] = {
                ...
                "total_discovery": int(global_buzz.get(symbol, {}).get("7d", 0))
                + len(global_buzz.get(symbol, {}).get("recent_news", [])),
                "analysts": obb.get("analyst_count"),
                "inst_pct": obb.get("inst_ownership_pct"),
                "short_pct": obb.get("short_interest_pct"),
                "conviction_count": research_raw.get(symbol, {})
                .get("human_research", {})
                .get("inst_13f_alpha", {})
                .get("conviction_count", 0),
                "rev_num": rev_num,
        ...
        for symbol in target_tickers:
            ...
            mom_bars = stats_map.get(symbol, {}).get("recent_7d_list", [0] * 7)
                ...
                **stats_map[symbol],
                "human_research": {
                    **r_info,
                    ...
                    "Target Upside": r_info.get("Target Upside")
                    or f"+{int(stats_map[symbol]['upside']*100)}%",
                    "Rev Growth Est": stats_map[symbol].get("growth_str")
                    or r_info.get("Rev Growth Est")
                    or (f"{stats_map[symbol]['rev_num']}%" if stats_map[symbol]["rev_num"] else ""),
                },
    ...
    def _generate_html(self):
        ...
        if "Independent Autonomous Mode" in html:
            html = html.replace(
                ...
                f"Independent Autonomous Mode | Last Generated: {ts}",
            )
