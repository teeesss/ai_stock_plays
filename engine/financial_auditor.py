...
async def audit_financials(csv_path, max_tickers=None):
    ...
    valid_tickers = [
        t for t, e in master_data.items() if e.get("human_research", {}).get("Status") != "Private"
    ]
    ...
    def get_ticker_variants(ticker):
        ...
        if ticker in m_map:
            variants.extend(m_map[ticker])
        if ":" in ticker:
            parts = ticker.split(":")
            variants.append(f"{parts[1]}.T") if "TYO" in parts[0] else None
            variants.append(f"{parts[1]}.DE") if "FRA" in parts[0] else None
        if ticker.endswith(".TW"):
            variants.append(ticker.replace(".TW", ".TWO"))
        if ticker.endswith(".TWO"):
            variants.append(ticker.replace(".TWO", ".TW"))
    ...
    try:
        for ticker, entry in master_data.items():
            if max_tickers and processed_count >= max_tickers:
                break
            ...
            for v_ticker in variants:
                print(
                    f"  [{ticker}] Trying {v_ticker}... ({processed_count + 1}/{len(master_data)})",
                    end="\r",
                )
                sys.stdout.flush()
