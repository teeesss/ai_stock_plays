...
# ─────────────────────────────────────────────────────────────
# GIGACPO Institutional 13F Fetcher [V28]
# ─────────────────────────────────────────────────────────────
            ...
            holders.append(
                {
                    ...
                    "is_conviction": any(
                        target.lower() in holder_name.lower() for target in CONVICTION_TARGETS
                    ),
                }
            )
...
def run():
    ...
    tickers = [
        t
        for t in master_data.keys()
        if master_data[t].get("human_research", {}).get("Bucket") != "Private"
    ]
    ...
    for ticker in tickers:
        ...
        if holdings:
            ...
            updated_count += 1
            log.info(
                f"  [OK] Updated ${ticker} | {len(holdings)} holders | {master_data[ticker]['human_research']['inst_13f_alpha']['conviction_count']} conviction"
            )
    ...
    if updated_count > 0:
        ...
        if JS_PATH.exists():
            js_content = (
                "window.CPO_MASTER_DATA = "
                + json.dumps(master_data, indent=2, ensure_ascii=True)
                + ";"
            )
            JS_PATH.write_text(js_content, encoding="utf-8")
            log.info("Synced dashboard JS bridge.")
