...
USER_AGENTS = [
    f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/160.0.8827.101 Safari/537.36",
    f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/160.0.8827.105 Safari/537.36",
    f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/160.0.8827.110 Safari/537.36"
]

class StealthNavigator:
    """
    Ultimate Stealth Navigator (V4.4)
    ...
    async def human_mouse_move(self, page, x, y):
        ...
        for i in range(steps):
            # Non-linear curve
            inter_x = current_pos['x'] + (x - current_pos['x']) * (i / steps) + random.uniform(-2, 2)
            inter_y = current_pos['y'] + (y - current_pos['y']) * (i / steps) + random.uniform(-2, 2)
        ...
        if links:
            ...
            for link in sample_links:
                try:
                    ...
                    if box:
                        await self.human_mouse_move(page, box['x'] + box['width']/2, box['y'] + box['height']/2)
    ...
    async def get_session_state(self, url, state_path=None):
        ...
        if not crumb:
            try:
                crumb = await page.evaluate("() => (window.App && window.App.main && window.App.main.context && window.App.main.context.dispatcher && window.App.main.context.dispatcher.stores.CrumbStore) ? window.App.main.context.dispatcher.stores.CrumbStore.crumb : ''")
            except:
                crumb = ""
