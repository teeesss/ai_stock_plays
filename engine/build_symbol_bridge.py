    ...
    try:
        ...
        for row in rows:
            ...
            if len(cols) >= 2:
                ...
                clean_name = re.sub(r' (Inc\.|Corp\.|Ltd\.|Co\.|Corporation|Incorporated|Common Stock)$', '', name, flags=re.I).strip()
    ...
    try:
        ...
        for line in lines:
            ...
            if len(parts) >= 2:
                ...
                if not ticker or len(ticker) > 6 or "-" in ticker or ticker.isdigit(): continue
                
                clean_name = re.sub(r' (Inc\.|Corp\.|Ltd\.|Co\.|Corporation|Incorporated|Common Stock)$', '', name, flags=re.I).strip()
                # Update but don't overwrite S&P (S&P has higher authority for mapping collisions)
                if name.upper() not in mapping: mapping[name.upper()] = ticker.upper()
                if clean_name.upper() not in mapping: mapping[clean_name.upper()] = ticker.upper()
    except Exception as e:
        print(f"  [ERR] Russell fetch fail: {e}")
            
    # Standard Overrides
    mapping["GOOGLE"] = "GOOGL"; mapping["ALPHABET"] = "GOOGL"; mapping["NVIDIA"] = "NVDA"
    mapping["META"] = "META"; mapping["FACEBOOK"] = "META"; mapping["TESLA"] = "TSLA"
    mapping["APPLE"] = "AAPL"; mapping["AMAZON"] = "AMZN"; mapping["MICROSOFT"] = "MSFT"
