...
def ultimate_reconstruct_v12_2(text):
        ...
        if text[i] == "$":
            ...
            for k in range(i, min(i + 15, len(text))):
                ...
                if char.isalnum() or char.isspace() or char in "/.":
                    ...
                    clean_upper = (
                        potential_str.upper().replace(" ", "").replace("/", "").replace(".", "")
                    )
    ...
    res = re.sub(
        r"(\$[A-Z0-9]{2,10})(WHICH|THAT|IS|WITH|FROM|AND|OF|OR|FOR|THE|TO)([A-Z]{1,})",
        r"\1 \2 \3",
        res,
    )
...
def process_translation(text):
    ...
    if re.search(r"[\u3040-\u30ff\u4e00-\u9faf\uac00-\ud7af]", text):
        ...
        # known manual fixes
        mapping = {
            "학위할때 포토닉스가 신생이기도했고 먼 미래 기술이라 생각했는데 졸업하고 마침 직장을 잡고보니 뜨고있는 것 같아 운이 좋다고 생각합니다. ㅎㅎ 말씀 감사드립니다": "When I was getting my degree, photonics was new and I thought it was a technology of the far future. After graduating and getting a job, it seems to be taking off, so I think I'm lucky. Haha, thank you for your words",
            "あなたのコミュニティとサブスクリプションはとても専門的で勉強になるので、日本の投資家の皆さんにも、ぜひ紹介したいです": "Your community and subscription are so professional and educational that I would love to introduce them to Japanese investors.",
            "섭스택에서 정말 잘 먹힐 것 같아서, 강력히 권유했는데, 정말 놀라울 정도로 빠르게 성장하셨습니다. 항상 유익할 글과 흥미로운 글 재밌게 잘 써주시는 담낭님 Kudos!!": "I strongly recommended it because I thought it would do really well on Substack, and you have grown surprisingly fast. Thank you for always writing such informative and interesting posts, Dam-nang-nim! Kudos!!",
        }
...
def run_repair():
    ...
    if history_log:
        ...
        print("Detailed repair log written to engine/repair_history_v12_2.txt")
