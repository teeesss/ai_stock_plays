...
async def _get_yf_supplement_stealth(ticker_symbol: str, entry: dict, client, crumb) -> dict:
    ...
    try:
        ...
        if target_mean is not None:
            result["analyst_target_mean"] = round(target_mean, 2)
        if target_high is not None:
            result["analyst_target_high"] = round(target_high, 2)
        if target_low is not None:
            result["analyst_target_low"] = round(target_low, 2)
        if n_analysts is not None:
            result["analyst_count"] = int(n_analysts)
        if buy_pct is not None:
            result["analyst_buy_pct"] = buy_pct
        if rec_mean is not None:
            result["rec_mean_raw"] = round(rec_mean, 2)
        if inst_pct is not None:
            result["inst_ownership_pct"] = round(inst_pct * 100, 1)
        if short_pct is not None:
            result["short_interest_pct"] = round(short_pct * 100, 1)
...
async def run_fetch_async(tickers: list = None, force: bool = False, dry_run: bool = False):
    ...
    for i, ticker in enumerate(tickers):
        ...
        if supplement:
            ...
            updated += 1
            log.info(
                f"  -> {ticker}: 1y={supplement.get('perf_1y')}% | analysts={supplement.get('analyst_count')}"
            )
        else:
            skipped += 1
        ...
        if updated > 0 and updated % 10 == 0:
            if not dry_run:
                for db_name, data in all_data.items():
                    path = (
                        DB_PATH
                        if db_name == "CPO_MASTER_DATA.json"
                        else ROOT / "database" / db_name
                    )
