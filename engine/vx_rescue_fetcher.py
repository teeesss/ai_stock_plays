...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "vx_rescue.log", encoding="utf-8"), logging.StreamHandler()]
)
...
GATEWAYS = [
    ...
    {"url": "https://fxtwitter.com/status/{id}", "type": "og_scrape"}, # HTML Fallback 1
    {"url": "https://vxtwitter.com/status/{id}", "type": "og_scrape"}, # HTML Fallback 2
...
def get_session():
    ...
    if _SESSION is None or _CALL_COUNT > 12:
        log.info(f"[STEALTH] Initializing primary Chrome 160 session profile")
...
def rescue_tweet(tweet_obj):
    ...
    if not author or author == "Twitter": author = tweet_obj.get("username", "Unknown")
    log.info(f"Rescued {tweet_id} from @{author}")

    # Media
    media = tweet_info.get("media_urls", []) or tweet_info.get("media", {}).get("photos", []) or vx_data.get("media_urls", [])
