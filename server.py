...
def _run_pipeline(pipeline: str = "full") -> dict:
    ...
    if not _sync_lock.acquire(blocking=False):
        return {"status": "busy", "message": "A sync is already running. Try again shortly."}
...
async def api_sync_full():
    ...
    if _sync_state["running"]:
        return JSONResponse({"status": "busy", "message": "Sync already in progress."}, status_code=409)
