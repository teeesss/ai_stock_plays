...
- **V28.8 [2026-04-29]**: **Sovereign Intelligence Dual-Dispatch & Data Hardening**. The intelligence engine now performs an atomic "Dual-Dispatch" of the market dossier to `bmwseals.com/email` via `RemoteSync`. Valuation logic hardened with `MCap:` terminology, crisp `'26 [9.9]` P/E formatting, and strict `[-500, 1000]` outlier filtration. Injected a permanent authoritative "Web Link" into the institutional header.
- **V28.8.10 [2026-05-01]**: **Anti-Fragile Intelligence & Archive Stabilization**. Transitioned the Market Synopsis pipeline to a multi-tier intelligence stack with `database/ai_intel_cache.json` as a primary fallback. Implemented **Session Gating** to restrict Edward Jones (EDJ) summaries to After-Hours (AH) sessions only. Integrated source-level timestamps (`[ SOURCE @ HH:MM EST ]`) into dossier rendering. Resolved archive 404 navigation errors by forcing absolute local path discovery and triggering synchronous `RemoteSync` for all dossier assets.
- **V28.8 CLOSED [2026-04-30]**: **Institutional Header Finalized**. Canonical sub-header format locked: `ESTABLISHED V28.8 // IDENTITY STANDARDIZED // HH:MM EST // WEB LINK` in `compose_html` (line ~2388). `render_tile` confirmed fully decoupled — zero extraneous header injections in individual ticker tiles. Architecture rule: ALL header-level branding changes must target the `<tr>` block in `compose_html` ONLY. All 3 test suites green (Syntax Audit, Layout Integrity V28, Regression Suite / 158 tests).
...
### 2026-05-05 - V30.6.10 Intelligence Pipeline Hardening
- **Extensionless URL Deployment**: Migrated the high-density ticker cockpit to an extensionless target `bmwseals.com/stocks/tickers`. This is handled by `engine/remote_sync.py` and `engine/ticker_dashboard.py`.
- **Session-Relative Extended Pricing**: Standardized `get_ticker_session_data` in `ticker_utils.py` to calculate percentage changes for AH, PRE, and OVN sessions relative to **today's regular market close**. This ensures that extended session data reflects momentum and direction within the session, while the "C %" column continues to track performance relative to yesterday's close.
- **Auto-Dependency Guardian**: Integrated `dependency_mgr` into `ticker_dashboard.py` and `ticker.sh` to automatically detect missing Python libraries (e.g., `curl_cffi`, `playwright`) and trigger automated recovery/installation without manual intervention.
- **Mobile UI Parity**: Implemented a responsive media query for the Ticker Cockpit that automatically collapses the 9-column grid to a 6-column high-density layout on mobile viewports (<800px). This prioritizes Ticker, Price, Session, % CHG, Close, and C % while hiding valuation metrics (MCap/PE).
- **Numerical Momentum Sorting**: Corrected the watchlist sorting engine to prioritize absolute numerical value for percentage changes (High-to-Low) rather than absolute magnitude, ensuring the top of the list reflects the strongest gainers.
- **Environment Bootstrapping**: Upgraded `ticker.sh` to a full venv-aware bootstrapper that validates the environment, syncs `requirements.txt`, and verifies Playwright binaries before launching the engine.
