...
class TestIntelligenceEngine(unittest.TestCase):
    def test_scores_are_dynamic(self):
        ...
        data_map = {"GOD": stock_a, "MID": stock_mid, "BLOAT": stock_b}
        ...
        print(
            f"MID Alpha: {scores_mid['alpha']} | Risk: {scores_mid['risk']} | Hidden: {scores_mid['hidden']}"
        )
        print(
            f"BLOAT Alpha: {scores_b['alpha']} | Risk: {scores_b['risk']} | Hidden: {scores_b['hidden']}"
        )
