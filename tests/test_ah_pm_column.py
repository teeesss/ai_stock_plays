...
class TestExtHoursExtraction(unittest.TestCase):
    ...
    def test_ah_takes_priority_over_pm(self):
        ...
        items = [_make_item("NVDA", post_price=950.0, post_pct=1.8, pre_price=940.0, pre_pct=0.7)]
    ...
    def test_vol_spike_still_computed(self):
        ...
        items = [
            _make_item("AEHR", volume=1_000_000, avg_vol=250_000, post_price=8.0, post_pct=1.5)
        ]
...
class TestHTMLAHPMColumn(unittest.TestCase):
    ...
    def test_ah_pm_column_header_present(self):
        ...
        self.assertIn("PM/AH", self.html, "No 'PM/AH' column header found in cpo_plays.html")
    ...
    def test_ext_price_in_row_template(self):
        ...
        self.assertIn("e.extPrice", self.html, "Row template missing e.extPrice reference")

    def test_ext_pct_in_row_template(self):
        """Row innerHTML must reference e.extPct for the AH/PM percentage."""
        self.assertIn("e.extPct", self.html, "Row template missing e.extPct reference")

    def test_ext_type_in_row_template(self):
        """Row innerHTML must reference e.extType for the AH/PM label."""
        self.assertIn("e.extType", self.html, "Row template missing e.extType reference")
    ...
    def test_live_prices_ext_price_binding(self):
        ...
        self.assertIn("live.ext_price", self.html, "buildEntries() missing live.ext_price binding")

    def test_live_prices_ext_pct_binding(self):
        """buildEntries() must bind live.ext_pct to extPct."""
        self.assertIn("live.ext_pct", self.html, "buildEntries() missing live.ext_pct binding")

    def test_live_prices_ext_type_binding(self):
        """buildEntries() must bind live.ext_type to extType."""
        self.assertIn("live.ext_type", self.html, "buildEntries() missing live.ext_type binding")
...
class TestLivePricesSource(unittest.TestCase):
    ...
    def test_post_market_price_fetched(self):
        ...
        self.assertIn("postMarketPrice", self.src, "live_prices.py missing postMarketPrice fetch")

    def test_pre_market_price_fetched(self):
        """live_prices.py must call item.get('preMarketPrice')."""
        self.assertIn("preMarketPrice", self.src, "live_prices.py missing preMarketPrice fetch")
