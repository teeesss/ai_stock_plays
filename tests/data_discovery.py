...
async def fetch_enriched_data(
    base_tickers=["NVDA", "CRDO", "BESIY", "KLIC", "ENTG", "ONTO", "DNPCY"],
):
    print(f"🚀 Phase III: Estimating + Weekly History for {len(base_tickers)} stocks")

    nav = StealthNavigator(headless=True)
    await nav.initialize()
    cookies_list, crumb = await nav.get_session_state(
        f"https://finance.yahoo.com/quote/{base_tickers[0]}"
    )
    ...
    for ticker in base_tickers:
        ...
        # 1. Weekly History (5y)
        url_chart = (
            f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker}?range=5y&interval=1wk"
        )
        ...
        try:
            ...
            if "quoteSummary" in summary_data and summary_data["quoteSummary"]["result"]:
                ...
                for yr in income:
                    if "totalRevenue" in yr:
                        revenue_history.append(
                            {
                                "date": yr["endDate"].get("fmt"),
                                "val": yr["totalRevenue"].get("raw"),
                            }
                        )
                ...
                if len(revenue_history) >= 2:
                    ...
                    if oldest_rev and curr_rev and n_years > 0:
                        rev_growth["cagr"] = round(
                            ((curr_rev / oldest_rev) ** (1 / n_years) - 1) * 100, 2
                        )
            ...
            enriched_results[ticker] = {
                "history": history,
                "estimates": estimates,
                "performance": enriched_results.get(ticker, {}).get(
                    "performance", {}
                ),  # Keep existing
                "revenue_trends": {"growth": rev_growth if "rev_growth" in locals() else {}},
