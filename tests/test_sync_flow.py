...
class TestSyncFlow(unittest.TestCase):
    def test_build_dist(self):
        ...
        result = subprocess.run([npm_cmd, "run", "build"], cwd=str(ROOT), capture_output=True, text=True)
        ...
        self.assertTrue((dist_path / "database" / "dashboard_data.js").exists(), "dashboard_data.js missing in dist")
        self.assertTrue((dist_path / "intel.js").exists(), "intel.js missing in dist")
