...
def test_momentum_data_completeness():
    ...
    for ticker, entry in data.items():
        ...
        if not status:
            ...
            failures.append(f"{ticker}: Invalid status format (expected list[7], got {type(status)})")
            
    print(f"\n[INFO] Checked {checked} tickers for momentum data.")
    assert not failures, f"Momentum data validation failed for {len(failures)} tickers:\n" + "\n".join(failures[:10])
