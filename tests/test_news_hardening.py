...
def test_rejects_blacklisted_personalities(agg):
    ...
    for title in bad_titles:
        assert agg.is_article_safe(title=title, link="https://reuters.com/news", source="Reuters") is False
...
def test_enforces_36h_hard_cutoff(agg):
    ...
    # 20 hours ago (Fresh)
    fresh_ts = now - (20 * 3600)
    assert agg.is_fresh_enough(fresh_ts) is True

def test_applies_24h_decay(agg):
    now = time.time()
    # 30 hours ago (should trigger 50% penalty)
    stale_ts = now - (30 * 3600)
    base_score = 100
...
def test_rejects_non_english_content(agg):
    ...
    for title in bad_titles:
        assert agg.is_article_safe(title=title, link="https://globalnews.com", source="Global") is False
...
def test_enforces_geographic_relevance(agg):
    ...
    assert agg.is_article_safe(title="India's RBL Bank profit jumps", link="https://reuters.com/india-bank", source="Reuters") is False
    
    # Preserve international news if it has a high-alpha tech anchor
    assert agg.is_article_safe(title="Taiwan Semiconductor (TSMC) sees AI surge", link="https://taiwannews.com/tsmc", source="Taiwan News") is True
