...
def test_est_fallback():
    ...
    assert est_rough.hour == est_engine.hour, f"Hour mismatch: {est_rough.hour} vs {est_engine.hour}"
    assert abs(est_rough.minute - est_engine.minute) <= 1, f"Minute mismatch"
...
def test_transition_logic():
    ...
    last_type = "PRE"
    curr_sess = "LIVE"
    session_changed = (last_type == "PRE" and curr_sess == "LIVE") or \
                     (last_type == "LIVE" and curr_sess == "AH") or \
                     (last_type == "UNKNOWN")
